...
Enemy::Enemy(const std::string& name, int hp, int damage, int defense, int projectile_defense, int level,
    ...
    : Entity(hp, damage, defense, projectile_defense, level), name(name), expReward(expReward), moneyReward(moneyReward), level(level),
...
int Enemy::attack(Entity* target) {
    ...
    int damage = target->takeDamage(dmg, attackType);
    return damage;
}
